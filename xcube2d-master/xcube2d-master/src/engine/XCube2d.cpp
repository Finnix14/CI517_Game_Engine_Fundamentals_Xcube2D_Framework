...
XCube2Engine::~XCube2Engine() {
	...
	if (myEngineSystemInstance) //shutdown MyEngineSystem
	{
		myEngineSystemInstance->Shutdown();
		myEngineSystemInstance.reset();
	}
...
std::shared_ptr<XCube2Engine> XCube2Engine::getInstance() {
	...
	instance->myEngineSystemInstance->Init(instance->audioInstance);
	return instance;
}